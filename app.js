...
/* ===================== ESTADO ===================== */
let APP = null;   // dados carregados de recipes.json/ingredients.json
let S = null;     // estado do utilizador (guardado em localStorage)
let saveTimer = null;
...
function defaultState(app) {
  return {
    ...
    checklist: [
      'Comprar mercado','Temperar carnes','Cozinhar arroz',
      'Cozinhar batata','Assar legumes','Fazer sopa',
      'Montar marmitas','Congelar'
...
function mergeState(saved, fresh) {
  ...
  // merge by id so new seed items don't vanish
  const merge = (seed, stored) => {
    if (!stored) return seed;
    const m = {}; seed.forEach(r => m[r.id] = r); stored.forEach(r => m[r.id] = r);
...
function totalLunchServings() {
  return curWeek().lunches.reduce((s, l) => {
    const r = findRecipe(l.id); return r ? s + r.servings * l.batches : s;
  }, 0);
}

function weeklyLunchCost() {
  return curWeek().lunches.reduce((s, l) => {
    const r = findRecipe(l.id); return r ? s + recipeCost(r, S.ingredients) * l.batches : s;
  }, 0);
}

function weeklyDinnerCost() {
  return curWeek().dinners.reduce((s, id) => {
    const d = findDinner(id); if (!d) return s;
    return s + recipeCost(d, S.ingredients) * (S.settings.people / Math.max(1, d.servings));
  }, 0);
}

function weeklyCost() { return weeklyLunchCost() + weeklyDinnerCost(); }

function costPerMarmita() {
  const n = totalLunchServings();
  return n > 0 ? weeklyLunchCost() / n : 0;
}

function monthSpend() {
  const ym = new Date().toISOString().slice(0, 7);
  return (S.invoices || []).filter(i => i.date.startsWith(ym)).reduce((s, i) => s + i.total, 0);
}
...
function buildShoppingList() {
  ...
  curWeek().lunches.forEach(l => {
    const r = findRecipe(l.id); if (!r) return;
    r.ings.forEach(i => add(i, l.batches));
  });
  curWeek().dinners.forEach(id => {
    const d = findDinner(id); if (!d) return;
    d.ings.forEach(i => add(i, S.settings.people / Math.max(1, d.servings)));
  });
...
function renderDashboard() {
  ...
  const wCost = weeklyCost(), mCost = wCost * 4.33;
  const perM = costPerMarmita();
  ...
  return `
  <div class="card">
    <div class="eyebrow">Configuração da semana</div>
    ...
    <div><label>Jantares/semana</label><input type="number" min="0" max="7" id="cfg-dinners" value="${S.settings.dinnersPerWeek}" style="max-width:120px;"></div>
  </div>
  <div class="g2">
    ...
    <div class="stat"><div class="v">${fmt(wCost)}</div><div class="l">Custo semanal total</div></div>
    <div class="stat"><div class="v">${fmt(mCost)}</div><div class="l">Custo mensal estimado</div></div>
    <div class="stat"><div class="v">${fmt(monthSpend())}</div><div class="l">Gasto real este mês (faturas)</div></div>
    <div class="stat"><div class="v">${fmt(lunchC)} / ${fmt(dinnerC)}</div><div class="l">Almoço / Jantar esta semana</div></div>
  </div>
  ${have < need ? `<div class="card warn"><span class="eyebrow danger-text">⚠️ Faltam marmitas</span><div class="muted">${have} de ${need} planeadas. Vai a Cardápio e adiciona mais receitas.</div></div>` : ''}
  <div class="card">
    <h3>Custo por categoria de compras</h3>
    ${Object.keys(ICAT).map(c => {
      const v = byCat[c] || 0; if (!v) return '';
      return `<div class="barwrap"><div class="barlabel">${ICAT[c]}</div><div class="barbg"><div class="barfg" style="width:${Math.round(v/maxC*100)}%;"></div></div><div class="barval mono">${fmt(v)}</div></div>`;
    }).join('')}
  </div>`;
}
...
function bindDashboard() {
  ...
  Object.keys(map).forEach(id => {
    ...
    if (el) el.onchange = () => { S.settings[map[id]] = Math.max(0, parseInt(el.value)||0); save(); render(); };
  });
}
...
function renderMenu() {
  ...
  <div class="card">
    ${wk.lunches.length === 0
      ? '<div class="empty"><span class="ic">🍽️</span>Nenhuma receita escolhida.<br><span class="muted">Usa "Gerar" ou vai a Receitas.</span></div>'
      : wk.lunches.map((l, i) => {
          const r = findRecipe(l.id); if (!r) return '';
          return `<div class="lunchpick">
            <div class="nm">${r.name}<div class="muted">${r.servings} porções/lote · ${fmt(recipeCost(r, S.ingredients))}</div></div>
            <button class="step-btn" data-adj="-1" data-i="${i}">–</button>
            <span class="mono" style="min-width:20px;text-align:center;">${l.batches}</span>
            <button class="step-btn" data-adj="1" data-i="${i}">+</button>
            <button class="x-btn" data-rml="${i}">✕</button>
          </div>`;
        }).join('')}
  </div>
  ...
  <div class="card">
    ${wk.dinners.length === 0
      ? '<div class="empty"><span class="ic">🌙</span>Nenhum jantar escolhido.</div>'
      : wk.dinners.map((id, i) => {
          const d = findDinner(id); if (!d) return '';
          return `<div class="lunchpick"><div class="nm">${d.name}<div class="muted">${DTYPES[d.type]||''}</div></div><button class="x-btn" data-rmd="${i}">✕</button></div>`;
        }).join('')}
  </div>

  <div class="section-title">📅 Planeamento — 52 semanas</div>
  <div class="card">
    <div class="weekgrid">
      ${Array.from({length:52},(_,i)=>i+1).map(w => {
        const has = S.weeks[w] && (S.weeks[w].lunches.length || S.weeks[w].dinners.length);
        return `<button class="${w===S.currentWeek?'active':''} ${has?'has':''}" data-week="${w}">${w}</button>`;
      }).join('')}
    </div>
  </div>`;
}
...
function bindMenu() {
  ...
  if (gb) gb.onclick = () => {
    const wk = curWeek();
    wk.lunches = generateLunches(S.recipes, S.weeks, S.currentWeek, neededMarmitas());
    wk.dinners = generateDinners(S.dinners, S.weeks, S.currentWeek, S.settings.dinnersPerWeek);
    save(); toast('Cardápio gerado para a Semana ' + S.currentWeek + ' 🎉'); render();
  };
  document.querySelectorAll('[data-week]').forEach(b => b.onclick = () => {
    S.currentWeek = parseInt(b.dataset.week); save(); render();
  });
  document.querySelectorAll('[data-adj]').forEach(b => b.onclick = () => {
    const i = parseInt(b.dataset.i), adj = parseInt(b.dataset.adj);
    curWeek().lunches[i].batches = Math.max(0, curWeek().lunches[i].batches + adj);
    save(); render();
  });
  document.querySelectorAll('[data-rml]').forEach(b => b.onclick = () => {
    curWeek().lunches.splice(parseInt(b.dataset.rml), 1); save(); render();
  });
  document.querySelectorAll('[data-rmd]').forEach(b => b.onclick = () => {
    curWeek().dinners.splice(parseInt(b.dataset.rmd), 1); save(); render();
  });
}
...
function renderLunchBank() {
  ...
  let list = S.recipes.filter(r => f === 'all' ? true : f === 'fav' ? r.fav : r.cat === f);
  return `
  <div class="pills">
...
function renderDinnerBank() {
  ...
  let list = S.dinners.filter(d => f === 'all' ? true : f === 'fav' ? d.fav : d.type === f);
  return `
  <div class="pills">
...
function bindRecipes() {
  ...
  document.querySelectorAll('[data-add]').forEach(b => b.onclick = () => {
    ...
    if (kind === 'lunch') {
      const ex = curWeek().lunches.find(l => l.id===id);
      if (ex) ex.batches++; else curWeek().lunches.push({id, batches:1});
    } else {
      curWeek().dinners.push(id);
    }
    save(); toast('Adicionado ao cardápio ✓'); render();
  });
  const nb = document.getElementById('btn-new-recipe');
  if (nb) nb.onclick = () => openRecipeModal(S.recipesTab);
}
...
function renderShoppingCardapio() {
  ...
  ${allDone ? '<div class="card success">✅ Tudo comprado! "Comprar mercado" marcado na checklist.</div>' : ''}
  ${list.length === 0 ? '<div class="empty"><span class="ic">🛒</span>Sem itens.<br><span class="muted">Gera um cardápio primeiro.</span></div>' : ''}
  ${Object.keys(ICAT).map(c => {
    const items = byCat[c]||[]; if (!items.length) return '';
    return `<div class="section-title">${ICAT[c]}</div><div class="card">
      ${items.map(i => {
        const key = i.name+'|'+i.unit, done = !!S.shoppingDone[key];
        const best = cheapestStore(i.name, S.ingredients);
        return `<div class="shop-item ${done?'done':''}">
          <input type="checkbox" data-shopkey="${key}" ${done?'checked':''}>
          <span class="nm">${i.name}${best?` <span style="font-size:11px;color:var(--teal);">(${STORES[best]})</span>`:''}</span>
          <span class="qty">${i.qty.toFixed(2)} ${i.unit}</span>
        </div>`;
      }).join('')}
    </div>`;
  }).join('')}`;
...
function renderPantryList() {
  ...
  ${Object.keys(ICAT).map(c => {
    const items = byCat[c]||[]; if (!items.length) return '';
    return `<div class="section-title">${ICAT[c]}</div><div class="card">
      ${items.map(p => `<div class="shop-item ${p.checked?'done':''}">
        <input type="checkbox" data-pantrychk="${p.id}" ${p.checked?'checked':''}>
        <span class="nm">${p.name}</span>
        <button class="x-btn" data-pantryrm="${p.id}">✕</button>
      </div>`).join('')}
    </div>`;
  }).join('')}`;
}
...
function bindShopping() {
  ...
  document.querySelectorAll('[data-shopkey]').forEach(el => el.onchange = () => {
    ...
    const list = buildShoppingList();
    if (list.length && list.every(i => S.shoppingDone[i.name+'|'+i.unit])) {
      const item = S.checklist.find(c => c.text.toLowerCase().includes('mercado'));
      if (item) item.done = true;
    }
    save(); render();
  });
  const sa = document.getElementById('shop-add-manual'); if (sa) sa.onclick = openExtraModal;
  document.querySelectorAll('[data-pantrychk]').forEach(el => el.onchange = () => {
    const p = S.pantry.find(p => p.id===el.dataset.pantrychk); if (p) p.checked = el.checked; save(); render();
  });
  document.querySelectorAll('[data-pantryrm]').forEach(b => b.onclick = () => {
    const i = S.pantry.findIndex(p => p.id===b.dataset.pantryrm); if (i>-1) S.pantry.splice(i,1); save(); render();
  });
  const pa = document.getElementById('pantry-add'); if (pa) pa.onclick = () => {
    const n = prompt('Nome do item:'); if (!n) return;
    S.pantry.push({id:'p_'+Date.now(), name:n.trim(), cat:'desp', checked:false}); save(); render();
  };
...
function renderMore() {
  const sub = S.moreTab;
  const tabs = [
    ['freezer','❄️ Freezer'],['checklist','📋 Checklist'],
...
function renderFreezer() {
  ...
  return `
  <div class="card">
    ${S.freezer.map(f => `<div class="freezer-item">
    ...
    <button class="btn sm olive block" id="add-freezer" style="margin-top:6px;">+ item</button>
  </div>`;
}
...
function renderChecklist() {
  return `
  <div class="card">
    ${S.checklist.map(c => `<div class="chk ${c.done?'done':''}">
...
function renderPrecos() {
  ...
  return `
  <div class="card">
    <input type="text" id="price-search" placeholder="🔍 procurar ingrediente…" value="${S._priceQ||''}">
  </div>
  ${list.map(ing => {
    const best = cheapestStore(ing.name, S.ingredients);
    return `<div class="card" style="padding:10px 13px;">
      <div style="font-weight:700; font-size:13.5px; margin-bottom:7px;">${ing.name}</div>
      <div class="g3">
        ${Object.keys(STORES).map(s => `<div>
          <label style="${best===s?'color:var(--success);font-weight:800;':''}">${STORES[s]}${best===s?' ✓':''}</label>
          <input type="number" step="0.01" min="0" data-pstore="${ing.id}" data-ps="${s}" value="${ing.prices[s]??''}" placeholder="—">
        </div>`).join('')}
      </div>
    </div>`;
  }).join('')}
...
function renderBatch() {
  ...
  let clock = 0;
  const timeline = ordered.map(r => {
    const start=clock; clock+=r.prep; const cookEnd=clock+Math.max(r.oven||0,r.pressure||0);
    return {r,start,prepEnd:clock,cookEnd};
  });
  const total = Math.max(...timeline.map(t=>t.cookEnd),0);
  return `
  <div class="g2" style="margin-bottom:8px;">
    <div class="stat"><div class="v">${Math.round(total)}min</div><div class="l">Tempo total estimado</div></div>
  ...
  <div class="card">
    ${ordered.map((r,i) => `<div class="timeline-step">
    ...
    <div class="muted" style="margin-top:8px;">💡 Começa pelas que têm mais forno/pressão — preparas as outras enquanto cozem.</div>
  </div>
  <div class="section-title">Cronograma</div>
  <div class="card">
    ${timeline.map(t => `
      <div class="kv"><span>${fmtMin(t.start)} — prep ${t.r.name}</span><span class="mono">${t.r.prep}min</span></div>
      ${t.r.oven||t.r.pressure?`<div class="kv"><span>${fmtMin(t.prepEnd)} — ${t.r.oven?'forno':'pressão'}</span><span class="mono">${t.r.oven||t.r.pressure}min</span></div>`:''}
    `).join('')}
    <div class="kv" style="border-top:1.5px solid var(--line);margin-top:4px;padding-top:8px;font-weight:700;">
      <span>🏁 Pronto</span><span class="mono">${fmtMin(total)}</span>
    </div>
  </div>
  <div class="section-title">Etiquetas para as marmitas</div>
  <div class="card" id="labels-wrap">
    ${items.map(r => Array.from({length:r.batches}).map((_,bi) => `
      <div class="label-box">
        <div class="lname">${r.name}</div>
        <div class="lmeta">Semana ${S.currentWeek} · Lote ${bi+1} · ${r.freezes?'❄️ Congelar':'🍽️ Consumir fresco'} · ${r.diff}</div>
      </div>`).join('')).join('')}
    <button class="btn sm ghost block" id="print-labels">🖨️ Imprimir etiquetas</button>
  </div>`;
}
...
function renderFaturas() {
  ...
  return `
  <div class="card">
    <div class="eyebrow">Gasto real este mês</div>
    <h2 style="margin:0;">${fmt(spend)}</h2>
  </div>
  <div class="card">
    <input type="file" accept="image/*" capture="environment" id="invoice-file" style="display:none;">
    <button class="btn block" id="invoice-scan-btn">📸 Digitalizar fatura (foto)</button>
    <div id="invoice-status" class="muted" style="margin-top:8px;"></div>
  </div>
  ${S._invoiceDraft ? renderInvoiceDraft() : ''}
  <div class="section-title">Histórico</div>
  ${invs.length===0?'<div class="empty"><span class="ic">🧾</span>Sem faturas ainda.</div>'
  :invs.map(inv=>`<div class="card"><div class="row between"><span style="font-weight:700;">${STORES[inv.store]||inv.store} · ${inv.date}</span><span class="mono">${fmt(inv.total)}</span></div><div class="muted" style="font-size:12px;">${inv.items.length} item(ns)</div></div>`).join('')}`;
}
...
function renderInvoiceDraft() {
  ...
  return `<div class="card highlight">
    ...
    ${d.items.map((it,i)=>`<div class="ing-row">
      <input type="text" data-di="${i}" data-df="name" value="${it.name}">
      <input type="number" step="0.01" data-di="${i}" data-df="price" value="${it.price}">
      <span class="muted" style="font-size:11px;">/kg</span>
      <button class="x-btn" data-dirm="${i}">✕</button>
    </div>`).join('')}
    <button class="btn sm ghost" id="draft-add" style="margin:6px 0;">+ item</button>
    <div class="row between" style="margin-top:8px;">
      <button class="btn sm ghost danger" id="draft-cancel">Cancelar</button>
...
function renderInspiracoes() {
  ...
  return `
  <div class="card">
    <div class="field"><label>Link do Instagram (ou descrição)</label><input type="url" id="insp-url" placeholder="https://instagram.com/p/…"></div>
    <div class="field"><label>Notas rápidas</label><input type="text" id="insp-notes" placeholder="ex: wrap de frango com molho de iogurte"></div>
...
function bindMore() {
  ...
  document.querySelectorAll('[data-fr]').forEach(b => b.onclick = () => {
    const f = S.freezer.find(x=>x.id===b.dataset.fr), adj = parseInt(b.dataset.adj);
    if (f) f.qty = Math.max(0, f.qty + adj); save(); render();
  });
  document.querySelectorAll('[data-frrm]').forEach(b => b.onclick = () => {
    const i = S.freezer.findIndex(f=>f.id===b.dataset.frrm); if(i>-1) S.freezer.splice(i,1); save(); render();
  });
  const af = document.getElementById('add-freezer'); if(af) af.onclick=()=>{
    const n=prompt('Nome (ex: Frango desfiado):'); if(!n) return;
    S.freezer.push({id:'f_'+Date.now(),name:n.trim(),qty:1,unit:'porções'}); save(); render();
  };

  // Checklist
  document.querySelectorAll('[data-chk]').forEach(el => el.onchange = () => {
    const c=S.checklist.find(c=>c.id===el.dataset.chk); if(c) c.done=el.checked; save(); render();
  });
  document.querySelectorAll('[data-chkrm]').forEach(b => b.onclick = () => {
    const i=S.checklist.findIndex(c=>c.id===b.dataset.chkrm); if(i>-1) S.checklist.splice(i,1); save(); render();
  });
  const ca=document.getElementById('chk-add'); if(ca) ca.onclick=()=>{
    const t=prompt('Novo item:'); if(!t) return;
    S.checklist.push({id:'c_'+Date.now(),text:t.trim(),done:false}); save(); render();
  };
  ...
  document.querySelectorAll('[data-pstore]').forEach(el => el.onchange = () => {
    ...
    if(v!==null&&v>0){ if(!S.priceHistory) S.priceHistory={}; const k=ing.name.toLowerCase(); if(!S.priceHistory[k]) S.priceHistory[k]=[]; S.priceHistory[k].push({date:new Date().toISOString().slice(0,10),store:el.dataset.ps,price:v}); }
    save();
  });

  // Batch print
  const pl=document.getElementById('print-labels'); if(pl) pl.onclick=()=>{
    const w=window.open('','_blank');
    w.document.write('<html><head><title>Etiquetas</title></head><body style="font-family:sans-serif;">'+document.getElementById('labels-wrap').innerHTML+'</body></html>');
  ...
  if(isb&&ifile){
    ...
    ifile.onchange=async()=>{
      ...
      statusEl.textContent='⏳ A ler a fatura…';
      try{
      ...
      }catch(e){ statusEl.textContent='⚠️ Não consegui ler. Adiciona os preços manualmente em Preços.'; }
    };
  }
  document.querySelectorAll('[data-di]').forEach(el=>el.oninput=()=>{
    if(S._invoiceDraft){ const it=S._invoiceDraft.items[parseInt(el.dataset.di)]; if(it) it[el.dataset.df]=el.dataset.df==='price'?parseFloat(el.value)||0:el.value; }
  });
  document.querySelectorAll('[data-dirm]').forEach(b=>b.onclick=()=>{
    if(S._invoiceDraft){ S._invoiceDraft.items.splice(parseInt(b.dataset.dirm),1); render(); }
  });
  ...
  const dsv=document.getElementById('draft-save'); if(dsv) dsv.onclick=()=>{
    ...
    items.forEach(i=>{
      const ing=S.ingredients.find(x=>x.name.toLowerCase()===i.name.toLowerCase());
      if(ing){ ing.prices[d.store]=i.price; }
    });
    S.invoices=S.invoices||[];
    S.invoices.push({date:d.date,store:d.store,items,total});
    S._invoiceDraft=null; save(); toast('Fatura guardada — preços atualizados! ✓'); render();
  };
  ...
  const isave=document.getElementById('insp-save'); if(isave) isave.onclick=()=>{
    ...
    S.inspirations.push({id:'insp_'+Date.now(),url,notes,date:new Date().toISOString().slice(0,10)});
    save(); toast('Inspiração guardada ✓'); render();
  };
  document.querySelectorAll('[data-insprm]').forEach(b=>b.onclick=()=>{
    const i=S.inspirations.findIndex(x=>x.id===b.dataset.insprm); if(i>-1) S.inspirations.splice(i,1); save(); render();
  });
  document.querySelectorAll('[data-insp2r]').forEach(b=>b.onclick=()=>{
    const insp=S.inspirations.find(x=>x.id===b.dataset.insp2r);
    S.moreTab=''; S.recipesTab='dinner'; S.tab='recipes';
    render(); openRecipeModal('dinner', insp);
  });
}
...
function openRecipeModal(kind, prefill) {
  _modalIngs = [{name:'',qty:0,unit:'kg',cat:'horti'}];
  ...
  bg.innerHTML = `<div class="modal">
    ...
    <h2 style="margin-bottom:14px;">${kind==='lunch'?'Nova receita de marmita':'Nova ideia de jantar'}</h2>
    ${prefill?`<div class="muted" style="margin-bottom:8px;">📌 Inspiração: ${prefill.url||prefill.notes}</div>`:''}
    <div class="field"><label>Nome</label><input type="text" id="m-name" value="${prefill?(prefill.notes||''):''}"></div>
    ${kind==='lunch'
      ? `<div class="field"><label>Categoria</label><select id="m-cat">${Object.keys(CATS).map(c=>`<option value="${c}">${CATS[c]}</option>`).join('')}</select></div>`
      : `<div class="field"><label>Tipo</label><select id="m-type">${Object.keys(DTYPES).map(t=>`<option value="${t}">${DTYPES[t]}</option>`).join('')}</select></div>`}
    <div class="g3">
      <div class="field"><label>Congela?</label><select id="m-freeze"><option value="1">Sim</option><option value="0">Não</option></select></div>
      <div class="field"><label>Prep (min)</label><input type="number" id="m-prep" value="15"></div>
      <div class="field"><label>Dificuldade</label><select id="m-diff"><option>fácil</option><option>médio</option><option>difícil</option></select></div>
    </div>
    ${kind==='lunch'?`<div class="g2"><div class="field"><label>Forno (min)</label><input type="number" id="m-oven" value="0"></div><div class="field"><label>Pressão (min)</label><input type="number" id="m-press" value="0"></div></div>`:''}
    <div class="field"><label>Porções</label><input type="number" id="m-serv" value="${kind==='lunch'?4:3}"></div>
    <div class="field"><label>Ingredientes</label><div id="m-ings"></div><button class="btn sm ghost" id="m-add-ing">+ ingrediente</button></div>
    <button class="btn block" id="m-save" style="margin-top:10px;">💾 Guardar</button>
  </div>`;
  ...
  document.getElementById('m-save').onclick = () => saveNewRecipe(kind, bg);
}
...
function renderModalIngs() {
  ...
  c.innerHTML = _modalIngs.map((ing,i) => `<div class="ing-row">
    ...
    <select data-mi="${i}" data-mf="unit">
      ${['kg','l','unid'].map(u=>`<option ${ing.unit===u?'selected':''}>${u}</option>`).join('')}
    </select>
    <button class="x-btn" data-mirm="${i}">✕</button>
  </div>`).join('')
  + `<datalist id="ing-list">${S.ingredients.map(i=>`<option value="${i.name}">`).join('')}</datalist>`;
  c.querySelectorAll('[data-mi]').forEach(el => {
    el.oninput = el.onchange = () => {
      const i=parseInt(el.dataset.mi), f=el.dataset.mf;
      _modalIngs[i][f] = f==='qty' ? parseFloat(el.value)||0 : el.value;
    };
  });
  c.querySelectorAll('[data-mirm]').forEach(b => b.onclick = () => { _modalIngs.splice(parseInt(b.dataset.mirm),1); renderModalIngs(); });
}

function saveNewRecipe(kind, bg) {
  const name = document.getElementById('m-name').value.trim();
  if (!name) { toast('Dá um nome à receita'); return; }
  const ings = _modalIngs.filter(i=>i.name.trim());
  const base = { id:(kind==='lunch'?'rc_':'dc_')+Date.now(), name, freezes:document.getElementById('m-freeze').value==='1',
    prep:parseInt(document.getElementById('m-prep').value)||0, diff:document.getElementById('m-diff').value,
    servings:parseInt(document.getElementById('m-serv').value)||1, season:'todo', fav:false, ings };
  if (kind==='lunch') {
  ...
  save(); bg.remove(); toast('Receita guardada! ✓'); render();
}
...
/* ===================== NAV + RENDER ===================== */
const TABS = [
  {id:'dashboard',ic:'📊',lb:'Painel'},
  {id:'menu',ic:'🗓️',lb:'Cardápio'},
  {id:'recipes',ic:'📖',lb:'Receitas'},
  {id:'shopping',ic:'🛒',lb:'Lista'},
  {id:'more',ic:'☰',lb:'Mais'},
];

function renderNav() {
  document.getElementById('nav').innerHTML = TABS.map(t =>
    `<button data-tab="${t.id}" class="${S.tab===t.id?'active':''}"><span class="ic">${t.ic}</span>${t.lb}</button>`
  ).join('');
...
function render() {
  document.getElementById('topsub').textContent = ({
    dashboard:'Painel', menu:'Cardápio', recipes:'Receitas', shopping:'Lista de compras', more:'Mais'
  })[S.tab]||'';
